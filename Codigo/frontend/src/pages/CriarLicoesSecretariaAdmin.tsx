...
import "../style/CriarLicoesSecretariaAdmin.css";
...
export default function CriarLicoesSecretariaAdmin() {
  ...
  const titulo = useMemo(() => {
    ...
    const viaQuery = new URLSearchParams(window.location.search)
      .get("titulo")
      ?.trim();
    return viaState || viaParam || viaQuery || "Criar Nova Lição";
  ...
  const salvar = () => {
    const payload = {
      ...
      campos: campos.map((c) => ({ rotulo: c.rotulo })),
    ...
    a.href = url;
    a.download = `${titulo.replace(/\s+/g, "_").toLowerCase()}.json`;
  ...
  useEffect(() => {
    if (showDeleteModal && confirmBtnRef.current) {
            ...
            <div className="cf-card-inner">
              ...
              <div className="cf-block">
                <div className="cf-add-row">
                  <span className="cf-label">Adicionar campo</span>
                  <button
                    type="button"
                  ...
                  {campos.map((c, idx) => (
                    <div key={c.id} className="cf-item">
                      <input
                        ...
                        placeholder="Ex.: Checkbox, Campo de texto, Anexo de arquivo"
                        value={c.rotulo}
              ...
              <div className="cf-block">
                <div className="cf-label-row">
                  <span className="cf-label">Anexar arquivo</span>
                </div>
              ...
              <div className="cf-actions">
                <button className="cf-save" onClick={salvar}>
                  Salvar
                </button>
                <button
