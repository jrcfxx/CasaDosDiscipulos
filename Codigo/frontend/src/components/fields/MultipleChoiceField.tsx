...
/**
 * MultipleChoiceField Component
 * Campo de múltipla escolha com alternativas dinâmicas para questões de quiz
 * Permite adicionar/remover alternativas e marcar a resposta correta
 */
...
const MultipleChoiceField: React.FC<SelectFieldProps> = ({
  id,
  name,
  label,
  value,
  ...
  const parseValue = (val: any): MultipleChoiceData => {
  ...
  const [data, setData] = useState<MultipleChoiceData>(() => parseValue(value));
  ...
  const handleCorretaChange = (index: number) => {
    // Apenas uma alternativa pode ser correta - desmarca todas e marca apenas a selecionada
    const novasAlternativas = data.alternativas.map((alt, i) => ({
      ...alt,
      correta: i === index,
    }));
  ...
  return (
    ...
    >
      ...
      <div className="multiple-choice-builder">
      ...
      <style>{`
        ...
        .alternativa-checkbox input[type="radio"] {
          cursor: pointer;
        }
